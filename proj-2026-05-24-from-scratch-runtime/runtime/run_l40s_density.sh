#!/usr/bin/env bash
# On-box W3 density sweep for a g6e/L40S (sm_89) DL Base AMI.
#
# Run after this runtime subset is rsynced to the g6e. The script downloads the
# W3 source artifacts from S3, verifies eps_manifest.json SHA256s, compiles native
# sm_89 autotune-off AOTI packages into artifacts_sm89/, builds density_main, and
# runs each N in a fresh process.
set -euo pipefail
IFS=$'\n\t'
...
S3_URI=${S3_URI:-"s3://nemotron-phase2-eps-419599258555/density"}
...
download_artifacts_from_s3() {
  log "downloading W3 source artifacts from $S3_URI"
