...
| **1.2b** | **Harden to a robust streaming component** (from the paired-review action list): dynamic/bucketed encoder for partial+short chunks (BLOCKER); byte-exact encoder via torch.export/dynamic (T2a + kills near-tie flip); incremental-STFT preproc (raw_audio_ring); per-session StreamState+reset; metadata assertions; range checks. **← NEXT** | `runtime/cpp/steady.*` | T1 over Tm∈{1,15,16,17,31,319,320,321} + 2-utterance(silence) + per-chunk exact-vs-eager | **R2** | 5090 |
