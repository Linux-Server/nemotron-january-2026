...
| F' | **AOTI accuracy-knob matrix** (TF32-off / deterministic / fusion-autotune / fallback) — recover T2a byte-exact or document failure w/ generated-kernel evidence; gates whether the compiled encoder can stay T2a or is T1-only | `runtime/aot_knob_matrix.py` | byte-exact recovered? (Y/N w/ evidence) | R1 | container/5090 |
