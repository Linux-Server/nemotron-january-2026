...
| **1.2b** | **C++ STREAMING ASSEMBLY**: port the verified Python streaming loop — per-geometry encoder exports (first drop=0 / steady drop=2, shape-frozen traces + partial last chunk) + ring buffers + decoder state-carry + cadence. **← NEXT — faithful translation of the now-verified Python streaming loop** | `runtime/cpp/steady.*` | T1 single-stream (== Python streaming / NeMo) | **R2** | 5090 |
