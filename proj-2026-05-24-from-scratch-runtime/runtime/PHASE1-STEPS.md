...
| 1.3b-enc | **Finalize encoder substrate** — dynamic-T torch.export INFEASIBLE (8× subsampling residue constraints; `1.3b-finalize-encoder-findings.md`) → **padded fixed-T buckets** (production-proven, byte-exact). Build `enc_finalize_aoti.pt2` (fixed-T, length-as-input, pad+slice) → wires Phase B + the real encoder-cache FORK_ASSERT | `runtime/export_finalize_*`, `aot_compile_finalize.py` | padded-bucket token-benign vs eager; Phase B e2e | **R2** | container/5090 |
