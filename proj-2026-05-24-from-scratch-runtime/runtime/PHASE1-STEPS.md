...
| 1.2b-wire | **AOTI-compile the T2a .pt2 → .so** — ✅ **DONE w/ FINDING** (`0.2b-aoti-findings.md`): AOTI is **NOT byte-exact** (Triton fp reassoc; recurrent cache_t 1.66e-2, reproduced packaging-invariant) → **T2a unreachable via AOTI in any language**; but **T1 token-exact** on the probe clip (19/20 steady chunks). Encoder bar for the native runtime = **T1, not T2a**. Build emits a clean noexecstack `.so` (link-flag fix, host-loadable). C++ `AOTIModelPackageLoader` = plumbing (numerics settled by the shared `.so`); folds into 1.3/1.4 | `runtime/aot_compile.py`, `aoti_t1_check.py` | py byte-exact compare (FAIL=finding) + host T1 | R1+R2(actionD review running) | container/5090 |
