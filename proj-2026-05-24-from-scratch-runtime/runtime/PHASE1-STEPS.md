...
| 1.2b-wire | **AOTI-compile the T2a .pt2 → .so + load in C++ + byte-exact compare** (container now built+verified: `nemotron-aoti:cu128`). Export first-chunk geometry too. **← unblocked, NEXT** | `runtime/cpp/` | C++ byte-exact vs eager | R1 | container/5090 |
