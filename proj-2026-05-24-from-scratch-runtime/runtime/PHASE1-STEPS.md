...
| 1.3b-enc-prod | **Productionize (folded from enc-scale review):** fail-closed routing (Python gate + C++); drop0 buckets/eager fallback + out-of-range eager fallback; C++ corpus runner; unify+validate-all strip; artifact metadata/contract (model-id/rc/T/digest); build hardening (retry+self-check) | `runtime/*finalize*`, `cpp/finalize_main.cpp` | fail-closed + fallback + C++ corpus + metadata | **R2** | container/5090 |
