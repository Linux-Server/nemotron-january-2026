...
- [ ] **8. Client vad_stop debounce + finalize trigger** (Opus review — v5 demoted from PAIRED)
...
| 5 | StatsCollector Python-exact | done | (pending) | Opus review. New signature: record(SessionTiming, bool emitted) + set_admission(DensityAdmission*) + snapshot_json with Python-exact /stats shape per Step 1 audit. Completion predicate `!was_suppressed && vad_stop_ts && final_sent_ts`; 5 derived SLO metrics computed inside record() from raw SessionTiming. Admission mapping: direct for offered/admitted/shed_close_count/backlog_peak; stubbed for per-session ready-queue fields (Step 9 will wire). Smokes 7/7 PASS + N=200 0/200. Codex log: codex-jobs/step-05-stats-collector-bqp3ostdp.log. |
...
| 8 | Client vad_stop debounce + finalize trigger | pending | — | Opus review (v5 demoted from PAIRED). Per-session state machine + timer in recv-loop. No Silero (Python server has none either; client/Pipecat runs VAD). |
