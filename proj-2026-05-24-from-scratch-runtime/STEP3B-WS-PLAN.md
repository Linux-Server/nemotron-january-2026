...
- [x] **10. Graceful shutdown + backpressure** (Opus review)
...
| 11 | Test oracle run_compat.py + canonicalized diff | done (correctness; perf gate RESOLVED later) | (this commit) | PAIRED (Codex 3-round nsys analysis + Opus review). `tests/server_compat/run_compat.py` + spec doc. **Correctness PASS**: 8/8 utts byte-exact (event count + type/text/is_final/finalize flag + collector_text + 9-key finalize_timing), invalid-model + invalid-`last` checks PASS on both servers. Harness fixes: `--server-start-timeout-s` 300→600 (C++ AOTI cold-load ~5.5min); tolerate `density_main` rc≠0 sweep verdict when telemetry present; **WS perf measured paced** (realtime) to match density's paced baseline. Historical Step-11 result: **WS-overhead perf gate known FAIL** (ws_p95 42.7ms − density_p95 13.9ms = 28.9ms > 2ms bar) — NOT a finalize regression: per-utt finalize was 7.4–8.3ms (density parity, fixed via InferenceExecutor); the 28.9ms was N=8 concurrent finalizes serializing on the single un-scheduled inference thread (per-session ttfs [7.9,8.3,8.5,8.6,9.1,19.7,20.3,42.7] — 5/8 at parity, 3 queue). **RESOLVED in `proj-2026-05-29-1336` Steps 1-7**: scheduler is now the real steady path; final oracle `20260530T043055Z` PASS with ws_p95=9.909ms, density_p95=12.797ms, overhead=-2.888ms <= 2.000ms, and N=8 ttfs [8.747,9.628,8.382,8.620,9.560,8.078,9.455,9.909] with the old 42.7ms tail eliminated. |
...
- **★ HEADLINE — RESOLVED (this commit): finalize latency ~230 ms → ~8 ms.** Root cause =
  ...
  (was ~230 ms). **Residual (RESOLVED in `proj-2026-05-29-1336`):** at N=8 concurrent, the old
  executor path serialized finalizes (per-session ttfs tail 19.7/20.3/42.7 ms) and failed the
  WS-overhead perf gate (28.9 ms > 2 ms bar). The deferred **batched-steady scheduler integration**
  is now wired: `scheduler_enabled=false` is no longer the steady path, the Step-7 oracle passes
  (ws_p95=9.909ms, density_p95=12.797ms, overhead=-2.888ms), and the shadow gate proves
  scheduler-vs-inline token/event equivalence (`STEADY_SHADOW_REPORT verdict=PASS`). Original investigation below for reference. `ws_server.cpp:1917` spawns a **new `std::thread` per
