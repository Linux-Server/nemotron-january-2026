...
run_one(){
  ...
  ssh -i "$KEY" $SSHO ubuntu@"$IP" "cd ~/nemotron && CUDAGRAPH_MAX_B=$CUDAGRAPH_MAX_B FINALIZE_PROFILE=${FINALIZE_PROFILE:-0} BATCH_FINALIZE=${BATCH_FINALIZE:-0} BARRIER_DRAIN=${BARRIER_DRAIN:-0} FAULTHANDLER=${FAULTHANDLER:-0} CUDAGRAPH_FINALIZE=${CUDAGRAPH_FINALIZE:-0} GC_PROFILE=${GC_PROFILE:-0} bash start_prod_server.sh > srv_prod.log 2>&1" &
  local SSHSRV=$!
