...
- [x] **9. Local validation — byte-exact + latency + knee (compile×batch matrix)** (MAX_SIZE optimum pending the queued sweep)
...
- [x] **10. Modal re-sweep — knee/$ vs the batch=1 baseline** (T4+L4; NEGATIVE result — see note)
...
- [x] **11. Consolidate — docs + recommendation + memory**
...
| 8 | 5c: variable-B + fail-closed + memory + telemetry | **done** | 3b4f3bd | Memory gate B=1→16: reserved 2.6→3.1GB, **retained-after-churn=0** (clone-on-scatter frees leaving streams). **Startup device-aware cap** (`_configure_batch_memory_cap`: device_cap=46 on this box, clamps NEMOTRON_BATCH_MAX_SIZE to 80%-headroom). **Fail-closed** (`_disable_batching` reverts decoder→greedy + B=1): EOU+batch, CTC/hybrid (RNNT-purity check), beam, cuda-graph-decoder, unsafe-stack → B=1, no crash. Variable-B/churn: 18 streams join/leave+permute byte-exact, 520 fair requeues. Telemetry: fallback counts/reasons, group key, prompt lang, preproc/model/scatter ms, CUDA mem+retained. `test_batch_memory_churn.py`. |
| 9 | Local validation (compile×batch matrix + latency + knee) | **done** | c629070 | `local-validation.md` + `english_baseline_fp32.json` + harness ext (`7864feb`). **TF32 RESOLVED: greedy_batch+TF32-off B=1 is 8/8 BYTE-EXACT vs the committed (greedy+TF32-on) baseline → no re-baseline divergence; TF32-off cost +1.8% (10.24→10.42ms).** Knee matrix: A(B=1)=16, **B(batch-only)=40 (2.5×)**, C(compile-only)=24, D(compile+batch)=40 (compile doesn't move the knee + 124s warmup → compile OFF default). Latency gate pass (final TTFS p95 ~16ms). **200/200 strict byte-exact canary** (B=4-dominant batches). Rec: batch-only prod (greedy_batch, TF32-off, scheduler+batch on, compile off). **MAX_SIZE sweep (`max-parallelism-sweep.md`): forced-batch GPU ceiling ≥220 realtime-stream-equiv at B=46 (T(B) still SUBLINEAR — not GPU-saturated; bound is the conservative memory cap, not compute). Achievable realtime knee by MAX_SIZE: 4→40, 8→48, 16→52, 32→56 (N=1 latency unchanged ~17ms, client NOT the limiter). Gap 56/220=25% = independent-arrival/dispatch inefficiency (avg B only ~4 at the knee), NOT GPU/client. → raised defaults MAX_SIZE 4→32, MAX_WAIT 5→8ms. Modal (10) push N to 48-200.** |
| 10 | Modal re-sweep (knee/$ vs baseline) | **done — NEGATIVE** | _pending_ | `modal-resweep.md` + RESULTS.md. T4+L4 batched (greedy_batch, TF32-off, MAX_SIZE=32→capped 19/31). **Batching does NOT help the cloud: T4 knee 5→4 (0.8×, regressed — greedy_batch+TF32-off slower at B=1), L4 5→6 (1.2×).** At the cloud knee (~5) avg B≈1 (`{1:124,2:1}`) — too few concurrent streams to fill batches (need ~250). The local 3.5× does NOT transfer: the slow-CPU cloud's low knee is exactly where batching can't engage. Client not bound (send-overrun 14-16ms). Apps stopped. **→ cloud lever is COMPILE (single-stream launch-collapse), untested on cloud; or cheapest-GPU horizontal scale.** |
| 11 | Consolidate docs + recommendation | **done** | _pending_ | `SUMMARY.md` (full arc: 16→56 3.5× local, byte-exact; batching = self-host lever, NO cloud benefit; lane analysis + barrier finding + phasing 2×; production rec; next-phase lane-fixes). Memory: `streaming-batching-outcome.md` + refined `modal-asr-knee-launch-bound`. PLAN complete (Steps 0-11); lane-fixes (CUDA-graphs/parallel-lanes) are a follow-on phase. |
