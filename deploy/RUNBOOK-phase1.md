...
If you use a different SSO profile, set both `AWS_PROFILE` for the AWS CLI and
`NEMOTRON_AWS_PROFILE` for `ec2_up.py`:

```bash
export AWS_PROFILE=my-sso-profile
export NEMOTRON_AWS_PROFILE="$AWS_PROFILE"
...
{
  "UserId": "...",
  "Account": "419599258555",
  "Arn": "arn:aws:sts::419599258555:assumed-role/..."
}
```
...
installed. The smoke client lives in
[../proj-2026-05-19-eou-endpointing/run_full1000_conc12.py](../proj-2026-05-19-eou-endpointing/run_full1000_conc12.py);
backend and LB rsync exclusions intentionally do not copy that project.
...
Run the cloud smoke. Use a unique model tag so benchmark artifacts do not
clobber older results:

```bash
SMOKE_TAG="phase1_smoke_$(date +%Y%m%d_%H%M%S)"
python proj-2026-05-19-eou-endpointing/run_full1000_conc12.py \
  --url "$LB_URL" \
  --concurrency 5 \
  --limit 10 \
  --model-tag "$SMOKE_TAG"
...
```text
Running 10 samples at concurrency 5 (model_tag=phase1_smoke_20260527_143012)

Completed in 12s. ok=10 errors=0 timed_out/empty=0
TTFB (speech-end->final, ms): mean=... p50=... p95=... p99=...
server finalize (vad_stop->final, ms): p50=42.0 p95=...

Wrote 10 rows to .../stt-benchmark/stt_benchmark_data/results.db
Wrote .../proj-2026-05-19-eou-endpointing/full1000_phase1_smoke_..._results.json
```

Expected TTFS interpretation: `server finalize (vad_stop->final, ms)` p50
should be `<= ~60ms` on this small smoke. If errors show WebSocket close `1013`,
server-side admission fired; lower `--concurrency`, verify per-box active
streams with [drain.sh](drain.sh) `status`, and consider
