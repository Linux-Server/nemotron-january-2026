...
GEMMA4_MTP_PATCH = REPO_ROOT / "patches" / "apply-vllm-gemma4-mtp-suppress-tokens.py"

# 0.25.0 is the first release whose speculative config maps the drafter's
# `gemma4_unified_assistant` model_type onto gemma4_mtp. Before that the 12B
# drafter falls through to the Transformers backend, which has no AutoModel
# entry for Gemma4UnifiedAssistantConfig.
vllm_image = (
...
SPECULATIVE_MODEL_NAME = "google/gemma-4-12B-it-assistant"

# Voice turns are short: system prompt + a handful of dialogue turns. Keeping this
...
@app.server(
    ...
    min_containers=1,
    # First boot pulls 24GB of weights and runs torch.compile + CUDA graph capture,
    # which does not fit in the 10 minute default.
...
class Server:
    ...
    def start(self):
        ...
        cmd = [
            ...
            "--async-scheduling",
            # Without this, max_model_len defaults to the config's 262144 and the
            # 8 full_attention layers alone want ~17GB of KV -- more than an L40S
