...
- [x] **2. Warm `InferenceLane` pool (the substrate); replace the single executor** `[R2:C1/M7][R1:C3]`
...
- [x] **3. Finalize on the lane pool (synchronous) + shared finalize loader pool + snapshot/commit split** `[R2:C3/M6][R1:C3/C4/M8]`
...
- [x] **4. Single scheduler owner: un-gate + de-duplicate** `[R1:C5]`
...
- [x] **5. Shadow parity gate (isolated): prove scheduler == inline** `[R1:M6][R2:M5]`
...
| 2 | Warm InferenceLane pool; replace single executor | done | 4224156 | InferenceLane (own preproc/joint/predict + stream) replaces executor; session acquires lane on ctor/releases on dtor (SESSION-BOUND); enc_first shared+locked (enc_first_mutex); big-module-dup runtime guard (per_lane 46MB, shared enc_first/enc_steady/finalize); dead *_long_check removed; K=NEMOTRON_WS_LANES=admission cap; lanes warmed 160 iters, buckets 32/32. K=1 inert: b2-t1 0/0, runtime-smoke 0/0, oracle 8/8. Opus review: clean. |
| 3 | Finalize on lane pool (sync) + shared loader pool + snapshot/commit | done | 000a53e | finalize sync on session's bound lane (own modules+stream, stream-aware AOTI); shared FinalizeBucketLoaderPool num_runners=F (NEMOTRON_WS_FINALIZE_RUNNERS, indep of K; loader 0MB/shared 2364MB); snapshot/compute/commit split — gen-checked commit gates ALL parent mutation (generation atomic; stale→drop no corruption); enc_first locked+lock_wait telemetry. K>1 enabled. b2-t1 0/0, runtime-smoke 0/0, K=8 oracle 8/8, stats-smoke PASS; K×F matrix fork_flush 9-23ms no staircase, enc_first_lock_wait 65-68ms separate. Opus review: clean (gen-check correct). |
| 4 | Single scheduler owner: un-gate + de-dup | done | 9319109 | SharedRuntime is sole BatchedSteadyScheduler+LoaderSet owner; ws_server state_->scheduler/loader REMOVED; /scheduler_telemetry→SharedRuntime; NEMOTRON_WS_SCHEDULER (default on) replaces NEMOTRON_DENSITY_BATCH_STEADY; runtime guard THROWS on duplicate-owner; config-print+run_compat env updated; steady STILL inline. Single-owner log proof: scheduler_instances=1 steady_loader_sets=1 policy=ws_server count=0. b2-t1 0/0, runtime-smoke 0/0, oracle 8/8. Opus review: clean. |
| 5 | Shadow parity gate (isolated) | done | e4b32cd | NEMOTRON_WS_STEADY_SHADOW env-gated; clones pre_decode→inline+scheduler clones, scheduler enqueued / inline run, both decoded independently, ONLY inline commits (scheduler throwaway); compares enc_out/cache_ch/t/ch_len max-diff + token/event equality per-B; report in SharedRuntime dtor; `apply_runtime_steady_outputs` = behavior-preserving extraction (shadow-OFF byte-exact). Shadow-OFF inert: runtime-smoke 0/0, oracle 8/8, b2-t1 6/6 cases 0 div. Shadow-ON parity GO: rows=409 max_enc_out=1.80e-4 max_cache_ch=2.89e-4 max_cache_t=2.75e-2 max_cache_ch_len=0 token_div=0 event_div=0; B>1 proven (b4=210, b_gt1=210); k1=199 k2=16 k3=150 k4=44; verdict=PASS. Opus review: clean (isolation/commit correct, CUDA event RAII, recurrent_cache=true on continuation). |
