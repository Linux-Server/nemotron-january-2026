...
- [x] **6. Full-1000 concurrency-12 + semantic WER (apples-to-apples; benchmark = en-US)**
...
| 6 | Full-1000 conc-12 + semantic WER (en-US) | done | 4ebf011 | WER 4.72%/4.84% vs EN 1.94%/1.95%; TTFS p95 245 vs 247ms (parity); 3 empties+10 catastrophic — NOT rc-related (rc0≈rc3); model-capacity tradeoff. step6-ml-comparison.md |
| 7 | Consolidate docs + final English re-validation | done | 8810f8f | `multilingual-support-summary.md` (process-per-model design, runtime isolation, handshake, guarded server.py changes, rc3-not-a-regression, run commands). English re-val **GO**: 100/100 byte-identical to baseline, FORK_ASSERT clean, handshake error/error/ready, `.venv-asr` runtime. Pointer added to canonical finding doc. |
